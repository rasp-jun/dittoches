...
최신 패치는 [COMBAT_STATS_AND_SKILLS.md](COMBAT_STATS_AND_SKILLS.md)의 육각 사거리 표시, 사거리 내 적 우선 공격, 장착 전후 능력치·스킬 피해 미리보기입니다. 장비를 선택하고 아군 위에 마우스를 올려 비교하며 자동 합성과 회수도 반영합니다. 기존 스킬 아이콘 우클릭으로 성급별 계수·현재 피해량을 확인합니다. `DigimonSkills.json`을 공유하는 온라인 서버도 함께 재시작해야 합니다. 최신 검증은 서버 59개, 계산 비교 1,458개, 솔로 실행 632개, 온라인 실행 51개입니다. 아래 44/440개 기록은 이전 장비 패치 때의 검증입니다.
