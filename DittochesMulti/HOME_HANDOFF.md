...
최신 패치는 [COMBAT_STATS_AND_SKILLS.md](COMBAT_STATS_AND_SKILLS.md)의 AD/AP 계수와 유닛별 기본 공격 사거리입니다. 스킬 아이콘 우클릭으로 성급별 계수·현재 피해량을 확인합니다. `DigimonSkills.json`이 솔로/온라인 기본 체력·공격력·공속·사거리·마나까지 정의하며 서버를 함께 재시작해야 합니다. 최신 검증은 서버 53개, 계산 비교 1,458개, 솔로 실행 605개, 온라인 실행 50개입니다. 아래 44/440개 기록은 이전 장비 패치 때의 검증입니다.
