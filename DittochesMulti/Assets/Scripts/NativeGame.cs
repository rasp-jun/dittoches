...
public sealed partial class NativeGame : MonoBehaviour
{
    ...
    private void DrawGame()
    {
        ...
        DrawTop();DrawLeft();DrawBoard();DrawRight();DrawBench();DrawShop();DrawSelectionGhost();DrawReportToggle();
    ...
    private void HandleUnitDrag()
    {
        ...
        {
            dragSource=-1;
            int cell=arena.HitCell(p),seat=arena.HitBench(p);
    ...
    private bool TryMoveLegend(Vector2 point)
    {
        ...
        if(PointerOverGuide(point))return false;EnsureArena();Vector3 world;
    ...
    private void DrawSelectionGhost()
    {
        ...
        Rect hint=new Rect(Mathf.Clamp(p.x+22,290,1110),Mathf.Clamp(p.y-76,195,710),225,64);
    ...
    private void DrawUnitDetail()
    {
        ...
        GUI.Label(new Rect(1390,405,470,28),"기여 시너지",header);UnitTraitLine(442,"속성",m.attr);UnitTraitLine(493,"계열",m.family);UnitTraitLine(544,"역할",u.def.role);
    ...
    {
        battling=true;lastReportRound=RoundLabel();showCombatReport=true;inspectedUnit=null;healthTrails.Clear();battleProgress=0;battleStartedAt=Time.unscaledTime;battleTimeRemaining=28f;combatPopups.Clear();selectedBoard=-1;scoutedRival=-1;SetupBattle();battleText=RoundType()=="크립"?"악당 디지몬 토벌 중":currentOpponent+" 테이머와 전투 중";
